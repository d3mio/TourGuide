...
function PlannerContent() {
  ...
  return (
    <section className="max-w-7xl mx-auto px-8 py-16">
      ...
      <h1 className="font-serif text-[2.5rem] mb-2">{t("plan_title")}</h1>
      <p className="text-[0.9rem] text-muted mb-8">{t("plan_sub")}</p>
      
      {/* Toggle */}
      <div className="inline-flex bg-surface border border-bordercolor rounded-lg p-1 gap-1 mb-10">
        <button 
          onClick={() => setCurrentMode("packages")}
          className={`px-5 py-2 text-[0.78rem] tracking-[0.06em] uppercase rounded-md transition-colors ${currentMode === "packages" ? "bg-accent text-white" : "text-muted hover:text-textcolor"}`}
        >
          {t("plan_tab_pkg")}
        </button>
        <button 
          onClick={() => setCurrentMode("custom")}
          className={`px-5 py-2 text-[0.78rem] tracking-[0.06em] uppercase rounded-md transition-colors ${currentMode === "custom" ? "bg-accent text-white" : "text-muted hover:text-textcolor"}`}
        >
          {t("plan_tab_custom")}
        </button>
      </div>
      ...
      {currentMode === "packages" && (
        <div>
          <div className="inline-flex gap-1 bg-bg border border-bordercolor p-1 rounded-md mb-6">
            {(["solo", "duo", "group"] as const).map(tier => (
              <button 
                key={tier}
                onClick={() => setCurrentTier(tier)}
                className={`px-3 py-1.5 text-[0.73rem] tracking-[0.05em] rounded transition-all ${currentTier === tier ? "bg-surface text-textcolor shadow-[0_0_0_0.5px_var(--color-bordercolor)]" : "text-muted hover:text-textcolor"}`}
              >
                {t(`tier_${tier}`)}
              </button>
            ))}
              ...
              return (
                <div key={pkg.id}>
                  <div 
                    ...
                    className={`bg-surface border border-bordercolor p-6 grid grid-cols-1 md:grid-cols-[1fr_auto] gap-4 items-center cursor-pointer transition-colors ${isExp ? "rounded-t-xl border-accent" : "rounded-xl hover:border-accent"}`}
                  >
                    <div>
                      <h3 className="text-[1.3rem] font-serif mb-1">{pkg.name}</h3>
                      <div className="flex flex-wrap gap-1.5">
                        {pkg.tags.map(tag => (
                          <span key={tag} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[0.7rem] tracking-[0.06em] font-medium border border-bordercolor text-muted uppercase">{tag}</span>
                        ))}
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-[1.4rem] font-serif">{currentTier==='group'?'From ':''}${price}</div>
                      <div className="text-[0.72rem] text-muted">{currentTier==='solo'?'per person':currentTier==='duo'?'for 2 persons':'per person (group)' }</div>
                    </div>
                  </div>
                  
                  {isExp && (
                    <div className="bg-surface border border-accent border-t-0 rounded-b-xl p-6 animate-[fadeIn_0.25s_ease]">
                      <p className="text-[0.85rem] text-muted mb-4">{pkg.desc}</p>
                      ...
                      <ul className="space-y-3">
                        {pkg.itinerary.map((day, i) => (
                          <li key={i} className="flex items-start gap-4 pb-3 border-b border-bordercolor text-[0.83rem] text-muted last:border-0 last:pb-0">
                            <div className="w-8 h-8 rounded-full bg-accentdim text-accent flex items-center justify-center text-[0.68rem] font-medium shrink-0">{i+1}</div>
                            <span className="mt-1">{day}</span>
                          </li>
                        ))}
                      </ul>
                      <div className="mt-6 flex gap-3">
                        <button onClick={(e) => { e.stopPropagation(); handleSaveDraft(pkg.name); }} className="px-4 py-2 bg-accent text-white rounded-md text-[0.72rem] font-medium tracking-[0.06em] uppercase hover:opacity-85">Save Trip Draft</button>
                        <button className="px-4 py-2 bg-transparent border border-bordercolor text-textcolor rounded-md text-[0.72rem] font-medium tracking-[0.06em] uppercase hover:border-accent hover:text-accent">Request Booking</button>
                      </div>
                    </div>
                  )}
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[240px_1fr] gap-8 items-start">
          <div className="sticky top-20 flex flex-col gap-1">
            {WIZARD_LABELS.map((label, i) => {
              ...
              return (
                <div key={label} onClick={() => setWizardStep(stepNum)} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-[0.8rem] cursor-pointer transition-all ${isActive ? 'bg-surface text-textcolor border border-bordercolor' : isDone ? 'text-muted hover:bg-surface/50' : 'text-muted opacity-50'}`}>
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[0.68rem] font-semibold shrink-0 ${isDone ? 'bg-accent text-white' : isActive ? 'bg-bordercolor text-textcolor' : 'bg-bordercolor text-muted'}`}>{stepNum}</div>
                  <span>{label}</span>
                </div>
              );
            })}
            ...
            {wizardStep === 3 && (
              <>
                ...
                <label className="relative inline-flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="text-[0.85rem]">{wizNet ? "Use Tour Guide's Partner Network" : "Independent Booking"}</span>
                </label>
